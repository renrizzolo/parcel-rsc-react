...
/** generates a routes file in the `outputPath` (defaulting to routes.ts in `rootDir`) from pages found in `pagesPattern` */
export async function generateRoutes({
  ...
  const routesTypePath = resolveOutputPath(rootDir, outputPath);

  const autoGeneratedString = `// This file is auto-generated by ${packageName}.`;
  const routesTypeContent = `/* eslint-disable */
  ...
  export const routeTree = ${JSON.stringify(routeTree, null, 2)} as const satisfies RouteNode;

  // lookup routes by PageProps url
  export const routesByPage = ${JSON.stringify(routesByPage, null, 2)} satisfies {
...
`;
  ...
  const outputDir = path.dirname(routesTypePath);
  ...
  if (fs.existsSync(routesTypePath)) {
    ...
    if (!oldContent.includes(autoGeneratedString)) {
      throw new Error(
        `Cannot overwrite existing routes file at "${routesTypePath}" because it was not generated by ${packageName}. \n Please remove the file and try again.`
      );
    }

    // only write the file if the content has changed
    if (oldContent === formatted) {
      log(`✅ ${path.basename(routesTypePath)} is up to date.`);
      return;
    }
  }
...
export function resolveOutputPath(
  rootDir: string,
  outputPath: string = "routes.ts"
): string {
  const resolved = path.resolve(rootDir, outputPath);
  if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
    return path.join(resolved, "routes.ts");
  }
