...
export function buildRouteTree(filePaths: string[]): RouteNode {
  ...
  if (
    filePaths.find((filePath) => removeExtension(filePath) === "index") ===
    undefined
  ) {
    throw new Error(
  ...
  for (const node of nodes) {
    const nodePathDir = getPathDir(node.path);

    let bestParent = rootNode;
    let bestMatchLength = 0;

    // helper to recursively find the parent this node should be inserted under
    function findParent(potentialParent: RouteNode) {
      const parentPathDir = getPathDir(potentialParent.path);

      if (parentPathDir !== null && nodePathDir !== null) {
        // check if the potential parent's directory is a prefix of the current node's directory.
        if (nodePathDir.startsWith(parentPathDir)) {
          const matchLength = parentPathDir.split("/").length;
    ...
    // recursively search for the best parent
    findParent(rootNode);
    // insert the node under the best parent found
    bestParent.children.push(node);
    // sort the children alphabetically by path
...
/**
 * get the directory part of a path if it has more than one segment
 * /one/two -> one/two
 * /one -> null
 * / -> null
 **/
function getPathDir(pathStr: string): string | null {
  if (pathStr.charAt(0) !== "/") {
    throw new Error(`Expected path to start with a slash: "${pathStr}"`);
  }

  // all paths start with a slash, remove it
  const dir = path.dirname(pathStr.substring(1));
  return dir === "." ? null : dir;
}
...
function createRouteNode(filePath: string): RouteNode {
  const path = removeExtension(filePath);
